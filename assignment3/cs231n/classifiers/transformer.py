...
class CaptioningTransformer(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim, wordvec_dim, num_heads=4,
        ...
        vocab_size = len(word_to_idx) #单词数量
        ...
        self.visual_projection = nn.Linear(input_dim, wordvec_dim) #输入图像特征vec映射至词vec长度
        self.embedding = nn.Embedding(vocab_size, wordvec_dim, padding_idx=self._null) #将词数字转换成词向量
        ...
        self.transformer = TransformerDecoder(decoder_layer, num_layers=num_layers) #多个decoder block串出decoder
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        #emdeb caption add position norm image fea
        captions_embed = self.positional_encoding(self.embedding(captions)) #N,T - N,T,E
        image_features = self.visual_projection(features).reshape(N,1,captions_embed.shape[2])
        #prepare tgt mask
        tgt_mask = torch.tril(torch.ones(T,T),diagonal=0)
        #decoder
        out = self.transformer(captions_embed,image_features,tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_patches_pos = self.positional_encoding(x_patches)
        out = self.transformer(x_patches_pos)
        out_pooled = torch.mean(out,dim=1)
        scores = self.head(out_pooled)
        logits = scores
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
