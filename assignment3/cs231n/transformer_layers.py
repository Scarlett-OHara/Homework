...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        ############################################################################
        j = torch.ones(max_len, embed_dim)*(torch.arange(0,embed_dim,dtype = torch.float32)//2*2)
        j = torch.pow(10000,-j / embed_dim)
        i = torch.ones(max_len, embed_dim)*(torch.arange(0,max_len,dtype = torch.float32).reshape(max_len,-1))
        pe[:,:,0::2],pe[:,:,1::2] = torch.sin(i*j)[:,0::2],torch.cos(i*j)[:,1::2]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = Q.reshape(N, S, self.n_head, self.head_dim).transpose(1, 2)  # (N, H, S, head_dim)
        K = K.reshape(N, T, self.n_head, self.head_dim).transpose(1, 2)  # (N, H, T, head_dim)
        V = V.reshape(N, T, self.n_head, self.head_dim).transpose(1, 2)  # (N, H, T, head_dim)


        K_T = K.transpose(3,2)
        scores = torch.matmul(Q,K_T)/torch.sqrt(torch.tensor(self.head_dim,dtype=Q.dtype))
        if attn_mask is not None:
            scores = scores.masked_fill(attn_mask[None, None, :, :] == 0, float('-inf'))
        scores = torch.softmax(scores,dim=-1)
        a = self.attn_drop(scores)

        output = torch.matmul(a,V).transpose(1,2).reshape(N,S,E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(N,C,self.img_size//self.patch_size,self.patch_size,self.img_size//self.patch_size,self.patch_size)
        #x = x.permute(0,1,2,4,3,5).reshape(N,C,self.img_size//self.patch_size,self.img_size//self.patch_size,
                                           #self.patch_size*self.patch_size)
        #x = x.reshape(N,C,self.num_patches,self.patch_size*self.patch_size)
        x = x.permute(0,1,2,4,3,5).reshape(N,C,self.num_patches,self.patch_size*self.patch_size)
        x = x.permute(0,2,1,3).reshape(N,self.num_patches,C*self.patch_size*self.patch_size)        
        out = self.proj(x)
