...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    out = torch.cat([out_left, out_right], dim=0)  # [2*N, D] #按维度0拼起来
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for m in range(2*N):
            if m!=k:
                sum_k+=torch.exp(sim(z_k,out[m])/tau)
            if m!=k+N:
                sum_k_N+=torch.exp(sim(z_k_N,out[m])/tau)
        total_loss += (-torch.log(torch.exp(sim(z_k,z_k_N)/tau) / sum_k)-torch.log(torch.exp(sim(z_k,z_k_N)/tau) / sum_k_N))
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    sim_matrix = (out @ out.T) / (torch.linalg.norm(out,dim=1,keepdim=True)@torch.linalg.norm(out,dim=1,keepdim=True).T)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return sim_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    posnom = torch.cat([sim_positive_pairs(out_left,out_right),sim_positive_pairs(out_left,out_right)],dim=0)
    posnom = torch.exp(posnom/tau)
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = None
    numerator = -torch.log(posnom / denom)
